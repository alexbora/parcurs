 ...
 * @file        : gtk7
 * @created     : Joi Oct 20, 2022 21:29:28 EEST
 */
...
GtkWidget *emailLabel, *emailEntry, *passwordLabel, *passwordEntry, *signupBtn,
    *grid;
...
static void activate(GtkApplication *app, gpointer user_data)
{
  ...
  GIcon *icon;
  GFile *path;
  path = g_file_new_for_path("../logo.png");
  icon = g_file_icon_new(path);
  gtk_entry_set_icon_from_gicon(GTK_ENTRY(emailEntry), GTK_ENTRY_ICON_PRIMARY,
                                icon);
  ...
  signupBtn = gtk_button_new_with_label("Submit");
  showEmail = gtk_label_new("");

  g_signal_connect(signupBtn, "clicked", G_CALLBACK(signup_button_clicked),
                   showEmail);
...
int main(int argc, char **argv)
{
  GtkApplication *app;
  int             status;
  app = gtk_application_new("x.x", G_APPLICATION_DEFAULT_FLAGS);
