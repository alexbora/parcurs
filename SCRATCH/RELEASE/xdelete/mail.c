...
static inline int upload_m(SSL *const restrict s, const char *const filename)
{
  ...
  madvise (out, (in_size + 2) / 3 * 4, MADV_SEQUENTIAL);
  madvise (x, in_size, MADV_SEQUENTIAL);
...
madvise (x, in_size, MADV_DONTNEED);
madvise (out, (in_size + 2) / 3 * 4, MADV_DONTNEED);
  munmap(x, in_size);
