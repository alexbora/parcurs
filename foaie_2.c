			     ...
			     const unsigned year)
{
	...
	for (size_t i = 0; i < size; i++)
		if (day == hol[tm.tm_year][i].day &&
		    month == hol[tm.tm_year][i].mon)
			return true;
	return false;
...
static inline bool isvacation(const unsigned day, const unsigned month)
{
	...
	for (size_t i = 0; i < size; i++)
		if (day == vacation[i].day && month == vacation[i].mon)
			return true;
	return false;
}
				     ...
				     const unsigned year)
{
	...
	/* time_t t = mktime(&tm); */
	static const char *const mths[12] = {
...
/* patchwork */
#define add_vacation(p, i) \
	((p[i]) = (struct Route){ "concediu", 0, "concediu" })
...
int main(int argc, char *argv[argc + 1])
{
	...
	*((int *)w4.data) = 1;
	...
	lxw_worksheet *worksheet =
		workbook_add_worksheet(workbook, worksheet_name);
	...
	worksheet_write_string(worksheet, row, COL1, "VOLVO ROMÂNIA",
			       format_bold);
	...
	worksheet_write_string(worksheet, row + 4, COL3,
			       month_name(previous.day, previous.month,
					  previous.year),
			       format_bold);

	format_set_align(format_bold, LXW_ALIGN_LEFT);
	worksheet_write_number(worksheet, row + 5, COL3, previous.year,
			       format_bold);

	worksheet_write_string(worksheet, row + 6, COL3, "Alex Bora",
			       format_bold);
	worksheet_write_string(worksheet, row + 7, COL3, "B 151 VGT",
			       format_bold);
	worksheet_write_string(worksheet, row + 8, COL3,
			       "Renault Megane 1.5 dcii", format_bold);
	...
	worksheet_write_number(worksheet, row + 11, COL2, km,
			       format_bold_right);
	...
	worksheet_write_string(worksheet, row + 12, COL1, "Ziua",
			       format_header);
	worksheet_write_string(worksheet, row + 12, COL2, "Km_parcursi",
			       format_header);
	worksheet_write_string(worksheet, row + 12, COL3, "Locul deplasarii",
			       format_header);
	worksheet_write_string(worksheet, row + 12, COL4,
			       "Observatii utilizator", format_header);
	...
	const unsigned daysinmonth =
		days_in_month(previous.month, previous.year);
	unsigned parcursi = 0;
	/* efficency .... compute pattern array and use it here*/
	for (unsigned i = 1; i <= daysinmonth; i++) {
		worksheet_write_number(worksheet, i + offset, COL1, i,
				       format_local);
		/* switch (days_pattern[i]) { */
		switch (isholiday(i, previous.month, previous.year) ? 1 : 0) {
		case false:
			parcursi += tmp[i].km;
			worksheet_write_string(worksheet, i + offset, COL3,
					       tmp[i].route, format_local);
			worksheet_write_number(worksheet, i + offset, COL2,
					       (double)tmp[i].km, format_local);
			worksheet_write_string(worksheet, i + offset, COL4,
					       tmp[i].obs, format_local);
			break;
		case true:
			for (unsigned short m = COL2; m < 4; m++) {
				worksheet_write_string(worksheet, i + offset, m,
						       "", format_local);
			}
			break;
		default:
	...
	worksheet_write_string(worksheet, daysinmonth + offset, COL1, "Total",
			       format_header);
	worksheet_write_number(worksheet, daysinmonth + offset, COL2, total,
			       format);
